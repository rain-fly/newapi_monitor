...
### Requirement: 定时调度器
系统 SHALL 每3分钟自动触发一次 API 可用性测试任务。

#### Scenario: 正常间隔触发
- **WHEN** 距上次测试任务执行已过3分钟
- **THEN** 系统 SHALL 发起新的测试任务

#### Scenario: 服务重启后恢复
- **WHEN** 监控服务重启
- **THEN** 系统 SHALL 立即执行一次测试，然后按3分钟间隔继续调度
...
### Requirement: 结果持久化
系统 SHALL 将每次测试结果持久化存储到数据库。

#### Scenario: 成功测试记录
- **WHEN** API 调用成功完成
- **THEN** 系统 SHALL 记录时间戳、可用状态为 true、响应延迟（毫秒）

#### Scenario: 失败测试记录
- **WHEN** API 调用失败或超时
- **THEN** 系统 SHALL 记录时间戳、可用状态为 false、错误信息、延迟（若有）
